...
function collectPlurals(arr) {
  return arr.filter(word => word.slice(-1) == "s")
}
...
function createArray(num, str) {
  return Array.from(str.repeat(num))
}
...
return arr.filter(val => typeof(val) == "object").length
}
...
function getEmptyTills(arr) {
  return arr.filter(till => till.checkout.length === 0 )
}
